...
export async function auditar(page) {
  return await page.evaluate(() => {
    ...
    add('Landmarks', '<main> único envolvendo o conteúdo principal', 8, hasMain ? 8 : 0,
      `main encontrados: ${landmarks.main}`);
    add('Landmarks', '<header> e <footer> presentes', 4,
      (landmarks.header ? 2 : 0) + (landmarks.footer ? 2 : 0),
      `header=${landmarks.header} footer=${landmarks.footer}`);
    add('Landmarks', '<section>/<article> usados para agrupar', 4,
      Math.min(4, (landmarks.section + landmarks.article) ? 4 : 0),
      `section=${landmarks.section} article=${landmarks.article}`);
    add('Landmarks', 'Regiões de busca/filtro com <form> ou role=search', 4,
    ...
    add('Headings', 'Nenhum nível pulado', 5, skips === 0 ? 5 : Math.max(0, 5 - skips * 2),
      `saltos=${skips} sequencia=[${hs.join(',')}]`);
    add('Headings', 'Seções com heading próprio', 4,
      (() => {
    ...
    const labelled = controls.filter(c => {
      ...
      return (id && document.querySelector(`label[for="${id}"]`)) ||
        c.closest('label') || c.getAttribute('aria-label') ||
        c.getAttribute('aria-labelledby') || c.getAttribute('title');
    });
    add('Formulários', 'Todo controle tem rótulo acessível', 12,
    ...
    add('Formulários', '<button> com type explícito', 4,
      btns.length ? Math.round((btnsTyped.length / btns.length) * 4) : 0,
      `${btnsTyped.length}/${btns.length}`);
    add('Formulários', '<fieldset>/<legend> em grupos de filtro', 4,
      q('fieldset').length ? 4 : 0, `fieldset=${q('fieldset').length}`);

    // ---------- 4. Tabela de dados (peso 20) ----------
    const tables = q('table');
    const t = tables[0];
    add('Tabela', '<thead>/<tbody> presentes', 4,
    ...
    add('Tabela', 'Cabeçalho ordenável focável por teclado', 3,
    ...
    add('Divitis', 'Baixa proporção de div/span', 10,
      ratio <= 0.15 ? 10 : ratio <= 0.3 ? 7 : ratio <= 0.45 ? 4 : 1,
      `${generic}/${all} = ${(ratio * 100).toFixed(1)}% genéricas`);

    // ---------- 6. Texto e microssemântica (peso 8) ----------
    add('Texto', 'Listas reais (<ul>/<ol>/<dl>) em enumerações', 3,
      q('ul,ol,dl').length ? 3 : 0, `listas=${q('ul,ol,dl').length}`);
    add('Texto', '<strong>/<em> em vez de <b>/<i>', 3,
      q('b,i').length === 0 ? 3 : 1, `b/i=${q('b,i').length} strong/em=${q('strong,em').length}`);
    add('Texto', '<time>/<abbr>/<dfn> para datas e siglas', 2,
    ...
    add('Dinâmico', 'Contador de resultados como aria-live', 4,
      q('[aria-live],output,[role=status]').length ? 4 : 0,
      `live/status/output=${q('[aria-live],output,[role=status]').length}`);
    add('Dinâmico', 'Estado vazio anunciável (não só display:none)', 3,
      (() => {
        ...
        const e = document.getElementById('empty');
        return e ? `role=${e.getAttribute('role')} aria-live=${e.getAttribute('aria-live')}` : 'ausente';
      })());
    ...
    const nota = score >= 90 ? 'A' : score >= 80 ? 'B' : score >= 70 ? 'C' : score >= 60 ? 'D' : score >= 50 ? 'E' : 'F';

    return JSON.stringify({
      score, nota, pontosTotal, pesoTotal,
      porCategoria, checks, landmarks, meta
    }, null, 2);
  });
}
